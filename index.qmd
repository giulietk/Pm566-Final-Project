...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# Prepare the plot data as per the provided code
...
# Create interactive scatter plots for each vitamin
interactive_plots <- list()  # List to store the interactive plots

for (vitamin in vitamin_vars) {
  # Basic scatter plot with a linear model
  scatter_plot <- ggplot(plot_data, aes_string(x = "Calories", y = vitamin, color = "DietType")) +
    geom_point(aes(text = paste("Calories:", Calories, "<br>Vitamin:", vitamin, "<br>Diet:", DietType))) +  # Interactive points with text
  ...
  # Convert ggplot to plotly interactive plot
  interactive_plot <- ggplotly(scatter_plot, tooltip = "text")  # Use 'text' for hover information
  
  # Store the interactive plot in the list
  interactive_plots[[vitamin]] <- interactive_plot
  
  # Print the interactive plot for this vitamin
