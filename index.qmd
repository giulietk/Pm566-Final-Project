...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# Recreate figure 5
create_scatter_plots_data <- function(data) {
  ...
  for (vitamin in vitamin_vars) {
    for (nutrient in nutrient_vars) {
      if (!nutrient %in% colnames(data)) next
  ...
  # Save the data as JSON to use in JavaScript
  jsonlite::write_json(plot_data_list, "scatter_plot_data.json")
}

# Call function to create the data and save to JSON
create_scatter_plots_data(extracted_data)
```
