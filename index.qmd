...
The median high calorie diet included substantially higher caloric intake than either the low calorie diet or those not on a caloric diet (2528 vs 1834 and 1821 cals, respectively), meaning participants intending to eat high amonts of calories are following their dietary plan. On the other hand, the low calorie diet had a slightly higher median than those not on a caloric diet (1834 vs 1821 cals), meaning the average participant on a low calorie diet actually ate more calories than those not intending to calorically restrict and were not following their intended diet.
...
Red dashed lines are used to represent the recommended B vitamins intake levels for men of 1.2 mg of B1, 1.3 miligrams (mg) of B2 and 1 mg of B6.
...
create_scatter_plots <- function(data) {
  ...
  recommended_values <- list(B1 = 1.2, B2 = 1.3, B6 = 1)
  
  plot_list <- list()  # To store multiple plots
  ...
  for (vitamin in vitamin_vars) {
    ...
    scatter_plot <- ggplot(plot_data, aes(x = Nutrient, y = Vitamin)) +
      ...
      theme_minimal() +
      facet_wrap(~Nutrient_Name, scales = "free", nrow = 2, ncol = 2) +
    ...
    plot_list[[vitamin]] <- interactive_plot
  }
...
# Shiny UI
ui <- fluidPage(
  titlePanel("Interactive Nutrient vs Vitamin Levels"),
  sidebarLayout(
    sidebarPanel(
      selectInput("vitamin", "Select Vitamin", choices = c("B1", "B2", "B6")),
      width = 3
    ),
    mainPanel(
      plotlyOutput("scatter_plot"),  # Placeholder for interactive plot
      width = 9
    )
  )
)
...
server <- function(input, output) {
  ...
  # Generate interactive plots based on selected vitamin
  plot_data <- create_scatter_plots(extracted_data)
  
  # Render the selected vitamin's plot
  output$scatter_plot <- renderPlotly({
    selected_vitamin <- input$vitamin
    plot_data[[selected_vitamin]]
  })
}
